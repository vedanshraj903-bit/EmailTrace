...
class MailboxWatcher:
    def __init__(self, settings: Settings, analyzer: Analyzer) -> None:
        ...
        self._wake = threading.Event()  # cuts a wait short when the dashboard asks for something
    ...
    def status(self) -> MailboxStatus:
        with self._lock:
            return MailboxStatus(
                ...
                error=self._error,
                notice=self._notice,
                scan_pending=self._scan_request > 0,
    ...
    # --- dashboard controls --------------------------------------------------
    # These only set flags; the watcher thread owns the IMAP connection and acts on them.

    def check_now(self) -> None:
        """Checks for new mail straight away, or retries a failed login without waiting."""
        self._wake.set()

    def pause(self) -> None:
        with self._lock:
            self._paused, self._state, self._error = True, "paused", None
        self._wake.set()
    ...
    def scan_recent(self, count: int) -> None:
        """Analyses the newest `count` messages already in the folder, skipping any analysed before."""
        with self._lock:
            self._scan_request, self._notice = count, None
        self._wake.set()
    ...
    def _run(self) -> None:
        while not self._stop.is_set():
            if self._paused:
                self._sleep(3600)  # resume() wakes it; mail that arrived meanwhile is picked up after
                continue
            try:
                self._watch()
    ...
    def _scan(self, conn: imaplib.IMAP4_SSL, last_uid: int) -> None:
        with self._lock:
            count, self._scan_request = self._scan_request, 0
        _, data = conn.uid("SEARCH", None, "ALL")
        # Newer mail than last_uid is left to _fetch_new so it is not analysed twice.
        uids = [u for u in map(int, (data[0] or b"").split()) if u <= last_uid][-count:]
        added = sum(self._analyze_uid(conn, uid, skip_known=True) for uid in uids if not self._stop.is_set())
        skipped = len(uids) - added
        notice = f"Scanned {len(uids)} recent email{'s' if len(uids) != 1 else ''}: {added} new"
        with self._lock:
            self._notice = notice + (f", {skipped} already analyzed or skipped." if skipped else ".")

    def _analyze_uid(self, conn: imaplib.IMAP4_SSL, uid: int, skip_known: bool = False) -> bool:
        ...
        if not raw:
            return False
        if skip_known and self._analyzer.repo.sha_in_use(hashlib.sha256(raw).hexdigest()):
            return False
        try:
